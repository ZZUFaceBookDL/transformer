...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        # Q = self._W_q(query).view(BATCH_SIZE, -1, self._h, key).transpose(1,2)  # Q: [batch_size, n_heads, len_q, d_k]
        # K = self._W_k(key).view(BATCH_SIZE, -1, self._h, key).transpose(1,2)  # K: [batch_size, n_heads, len_k, d_k]
        # V = self._W_v(value).view(BATCH_SIZE, -1, self._h, key).transpose(1,2)  # V: [batch_size, n_heads, len_v(=len_k), d_v]
        #

        # Scaled Dot Product
        self._scores = torch.matmul(queries, keys.transpose(-1, -2)) / np.sqrt(self._q)
